...
def check_churn():
    if request.method == "GET":
        ...
        prediction_input = [
            {
                ...
                "ads_listened_per_week": float(
                    request.form.get("ads_listened_per_week")
                ),
            }
        ]
        ...
        res = requests.post(
            predictor_api_url, json=json.loads(json.dumps(prediction_input))
        )

        if res.status_code != 200:
            app.logger.error("Prediction API returned error: %s", res.text)
            return (
                jsonify(error="Prediction API error", details=res.text),
                res.status_code,
            )
